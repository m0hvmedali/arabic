    ...
    blocks.forEach(block => {
        ...
        if (!newReq.includes("كلمة (") && !newReq.includes("جملة (") && !newReq.includes("المحل الإعرابي للجملة") && !newReq.includes("منع كلمة")) {
            ...
            for (let w of parens) {
                ...
                const patterns = [
                    new RegExp(`(?:ف|و)?\\(${ew}\\)\\s+(?:تعرب|مبتدأ|خبر|فاعل|مفعول|اسم|حال|تمييز|نعت|مضاف|مستثنى|بدل)`),
            ...
            if (word) {
                ...
                } else if (optionsStr.includes('من')) {
                    newReq = `بين نوع (من) في الجملة`;
        ...
        // Check if there is still "اختر الإعراب الصحيح للكلمة المطلوبة" or "بين نوع (لا)" etc but it has a word parsing
        if (newReq === `اختر الإعراب الصحيح للكلمة المطلوبة` || newReq === `بين نوع (لا) في الجملة` || newReq === `أعرب أو بين نوع كلمة (لعمري)`) {
